...
// 外界可以通过 TokenScanner 类的公共方法获取以下信息:

// 是否还有更多标记：使用 hasMoreTokens 方法来判断当前是否还有待读取的标记。
// 获取下一个标记：使用 nextToken 方法来获取并返回下一个标记作为字符串。
// 获取当前位置：使用 getPosition 方法来获取当前标记在输入流中的位置。
// 获取标记类型：使用 getTokenType 方法来获取指定标记的类型。
...
// 代码解释
// 创建了一个 TokenScanner 对象，并将字符串 "Hello, world! 123" 设置为其输入
// ignoreWhitespace() 调用后，忽略输入中的空格和制表符，将它们视为分隔符而不是独立的标记。
// scanNumbers() 调用后，将对以数字开头的标记进行特殊处理，将其解析为一个完整的数值。
// 默认情况下，nextToken() 方法将数字和字母一样对待，没有为数字提供特殊处理。
// 使用 hasMoreTokens 方法检查是否还有待读取的标记
// 使用 nextToken 方法获取下一个标记作为字符串，也就是将字符串进行分割
// 使用 getTokenType 方法，可以确定标记的类型是单词（WORD）还是数字（NUMBER），并据此进行相应的处理


// 总之，就是对一个字符串进行读取，分割成一个个小块