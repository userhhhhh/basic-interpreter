...
#endif
// 定义了一个名为 strlib 的字符串库，提供了一些有用的字符串函数。

// 将整数转换为对应的数字字符串：integerToString
// 将数字字符串转换为整数：stringToInteger
// 将浮点数转换为对应的字符串形式：realToString
// 将表示实数的字符串转换为对应的值：stringToReal
// 将字符串中的小写字母转换为大写字母：toUpperCase
// 将字符串中的大写字母转换为小写字母：toLowerCase
// 判断两个字符串（忽略大小写）是否相等：equalsIgnoreCase
// 判断字符串是否以指定的前缀开始：startsWith
// 判断字符串是否以指定的后缀结束：endsWith
// 去除字符串两端的空白字符：trim